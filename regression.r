...
reg1 <- lm(Petal.Length ~ Petal.Width, data=dd) 
print (reg1) 
summary(reg1)
...
lines(Petal.Width, reg1$fitted.values, col="red")


plot(Sepal.Width, Petal.Width)
reg2 <- lm(Petal.Width ~ Sepal.Width, data=dd) 
print (reg2) 
summary(reg2)
...
plot(Sepal.Width, Petal.Width)
# and without closing the plot window 
lines(Sepal.Width, reg2$fitted.values, col="red")

reg3 <- lm(Petal.Width ~ Sepal.Width+Species, data=dd) 
print (reg3) 
summary(reg3)
